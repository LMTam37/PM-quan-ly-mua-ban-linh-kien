...
public class createProduct extends JDialog implements ActionListener {
	private JLabel lblProductName, lblMFG, lblMFGer, lblQty, lblPrice;
	...
	private void createGui() {
		...
		txtMFGer.setBounds(168, 107, 158, 19);
		pnMain.add(txtMFGer);
		...
		qtySpinner.setBounds(168, 148, 158, 20);
		...
		priceSpinner.setBounds(168, 188, 158, 20);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnCreate)) {
			if (isEmpty(txtProductName) || isEmpty(txtMFGer)) {
				...
				if (flag == 0) {
					ProductDAO.getInstance().addProduct(txtProductName.getText(), new Date(dcMFG.getDate().getTime()),
							txtMFGer.getText(), (Integer) qtySpinner.getValue(),
							BigDecimal.valueOf((Double) priceSpinner.getValue()));
